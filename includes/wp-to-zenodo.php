...
class WP_to_Zenodo {
	...
	public function set_api_key( $env ) {
		...
		if ( '' !== $value ) {
			...
			} elseif ( defined( 'PROD_ZENODO_KEY' ) && 'prod' == $env ) {
	...
	public function zenodo_community_storage() {
		if ( array_key_exists( 'zenodo_community_storage', $_POST ) && ( '' != $_POST['zenodo_community_storage'] ) ) {
			// update_option('zenodo_api-key', $_POST['zenodo_api-key']);
		}
		$value = get_option( 'zenodo_community_storage', '' );
		?>
		<input name="zenodo_community_storage" type="text" id="zenodo_community_storage" value="<?php echo esc_attr( $value ); ?>" class="regular-text code">'
		<?php
		return '';
	}
	...
	public function register_api_key() {
		...
		add_settings_field(
			'zenodo_community_storage',
			'URL slug for your community, the part that follows `https://zenodo.org/communities/`. Comma seperated for multiple communities.',
			array( $this, 'zenodo_community_storage' ),
			'general',
			'default'
			// array( 'label_for' => 'zenodo_api-key' )
		);
